...
class TestIB24Dataset(unittest.TestCase):
    ...
    def _test_basic_stats(self, name):
        ...
        self.assertEqual(len(record.multsqueezes), len(soln.multsqueezes))
        for i in range(len(soln.multsqueezes)):
            r_msq = record.multsqueezes[i]
            a_msq = soln.multsqueezes[i]
        ...
        for i in range(len(a_path.activations)):
            ...
            self.assertEqual(r_act.skips, a_act.skips)
            self.assertEqual(r_act.measure, a_act.measure)
    ...
    def test_white_washed(self):
        self._test_basic_stats("August Burns Red - White Washed")
